...
namespace WeirdoPanel\Commands\CRUDActions;
...
abstract class CommandBase extends GeneratorCommand
{
    ...
    public function handle()
    {
        ...
        if ($this->isReservedName($this->getNameInput())) {
            $this->error("El nombre '{$this->getNameInput()}' Está reservado por PHP.");
            return false;
        }
        ...
        if ($this->alreadyExists($this->getNameInput()) and !$this->option('force')) {
            $this->line("<options=bold,reverse;fg=red> • {$this->getNameInput()} {$this->type} ¡ya existe! </> \n");
        ...
        $this->buildBlade();
        $this->line("<options=bold,reverse;fg=green> {$this->getNameInput()} {$this->type} creado con éxito. </> 🤙\n");
    }
